...
export const getMessageEntity = (
  ...
  const relevantEntity = entities
    ...
    if(!Boolean(relevantEntity?.url.startsWith(expectedUrl))) {
      return null
    }
...
export const suffixJanusStatelessHTML = (identifier: string) =>  suffixStatelessHTML(SUFFIX, identifier, )
export const getJanusMessageEntity = (context: BaseContext) => getMessageEntity(context, SUFFIX)
